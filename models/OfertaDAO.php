...
class OfertaDAO {
    ...
    public function crear(Oferta $o) {
        ...
        $stmt->execute([
            ':emp'=>$o->empresa_id, ':tit'=>$o->titulo, ':desc'=>$o->descripcion,
            ':tipo'=>$o->tipo, ':sal'=>$o->salario_referencial, ':mod'=>$o->modalidad,
            ':fcierre'=>$o->fecha_cierre, ':estado'=>$o->estado_oferta ?? 'activa'
        ]);
    ...
    public function actualizar(Oferta $o) {
        ...
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([
            ':tit'=>$o->titulo, ':desc'=>$o->descripcion, ':tipo'=>$o->tipo,
            ':sal'=>$o->salario_referencial, ':mod'=>$o->modalidad,
            ':fcierre'=>$o->fecha_cierre, ':estado'=>$o->estado_oferta, ':id'=>$o->id_oferta
        ]);
    }
